 ...
 *   - No Math.random() anywhere in src/v2/
 */
...
describe('Determinism Contract (§7)', () => {
  ...
  describe('no Math.random() in src/v2/', () => {
    it('no file in src/v2/ uses Math.random()', () => {
      const v2Dir = path.resolve(__dirname, '../../src/v2');
      const files = fs.readdirSync(v2Dir).filter(f => f.endsWith('.ts'));

      for (const file of files) {
        const raw = fs.readFileSync(path.join(v2Dir, file), 'utf-8');
